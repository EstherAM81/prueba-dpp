...
export async function getReferencia(slug: string) {
  ...
  return {
    ...
    dpp_status: getText(p["dpp_status"]),
    schema_version: getText(p["schema_version"]),
    espr_product_category: getText(p["espr_product_category"]),
    packaging_tipo: getText(p["packaging_tipo"]),
    packaging_recycled_pct: getText(p["packaging_recycled_pct"]),
    packaging_recyclability_class: getText(p["packaging_recyclability_class"]),
